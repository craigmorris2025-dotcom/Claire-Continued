...
function renderPlatformBrowser() {
  ...
  return `<div class="platform-browser">
    ...
    <iframe class="platform-browser-frame" src="${platformRenderUrl(platformBrowser.url)}" title="Platform browser render"></iframe>
  </div>`;
}
...
function render() {
  ...
  if (subNode) subNode.textContent = "Platform AI - ACS2 Platform - v19.61";
...
async function runPortfolioPath() {
  await fetchJson("/evaluate", {
    ...
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      raw_input: "Run the 30-stage lifecycle through the default signal, trend, thesis, and portfolio intelligence path using the local governed project files.",
      mode: "deterministic",
