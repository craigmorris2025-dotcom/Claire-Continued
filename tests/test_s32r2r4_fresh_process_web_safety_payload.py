...
def test_s32r2r4_dashboard_payload_exposes_safety_activation():
    ...
    assert payload.get("governed_payload_reconciliation") is not None
    assert payload.get("governed_web_safety_activation") is not None
...
def test_s32r2r4_dashboard_payload_status_still_healthy():
    client = TestClient(create_app())
