...
def build_capsule(queue_id: str, candidate_url: str | None, operator_ack: str | None, payload: dict) -> dict:
    return {
        "version": VERSION,
        ...
        "fetch_plan": {
            "queue_id": queue_id,
            "candidate_url": candidate_url,
            "state": "prepared_without_fetching",
            "network_request_performed": False,
            "body_read_performed": False,
            "runtime_truth_write_allowed": False,
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prepare an S37 review export fetch capsule without fetching.")
    ...
    capsule = build_capsule(queue_id, candidate_url, operator_ack, payload)

    if args.output_path:
        output_path = Path(args.output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(json.dumps(capsule, indent=2, sort_keys=True), encoding="utf-8")

    print(f"[S37_CAPSULE][OK] Queue id found: {queue_id}")
    print(f"[S37_CAPSULE][OK] Prepared plan without fetching for queue_id={queue_id}")
