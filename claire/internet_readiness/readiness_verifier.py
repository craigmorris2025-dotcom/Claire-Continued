...
ENV_KEYS = [
    ...
    "SERPAPI_API_KEY",
    "BING_SEARCH_API_KEY",
    "CLAIRE_INTERNET_ALLOW_UNKNOWN_DOMAINS",
    "CLAIRE_INTERNET_MAX_RESULTS",
    "CLAIRE_INTERNET_TIMEOUT_SECONDS",
    "CLAIRE_INTERNET_MAX_BYTES",
]
...
def env_status(root: Path) -> Dict[str, Any]:
    ...
    provider_declared = key_status["CLAIRE_SEARCH_PROVIDER"]["declared_in_env_example"] or key_status["CLAIRE_SEARCH_PROVIDER"]["declared_in_env_or_process"]

    allow_unknown = None
    for source in [env_file, env_example]:
        for line in source.splitlines():
            if line.strip().startswith("CLAIRE_INTERNET_ALLOW_UNKNOWN_DOMAINS="):
...
def determine_readiness(layer_status: Dict[str, Any], update_status: Dict[str, Any], env: Dict[str, Any], governance: Dict[str, Any], scheduler: Dict[str, Any]) -> Tuple[str, List[str], List[str]]:
    ...
    if not env["provider_declared"]:
        warnings.append("CLAIRE_SEARCH_PROVIDER is not declared.")
