...
EXPECTED_FRONTEND_ASSETS = [
    ...
    "frontend/command_center/modern/operator_action_click_bridge.js",
    "frontend/command_center/modern/operator_action_click_bridge.css",
]

DANGEROUS_ENV_TOGGLES = [
    "CLAIRE_ALLOW_RESPONSE_BODY_READ",
    "CLAIRE_ALLOW_BODY_READ",
    "CLAIRE_ALLOW_AUTONOMOUS_EXECUTION",
    "CLAIRE_ALLOW_AUTONOMOUS_CRAWLING",
    "CLAIRE_ALLOW_RUNTIME_TRUTH_MUTATION",
    "CLAIRE_ALLOW_RUNTIME_MUTATION",
    "CLAIRE_ALLOW_PACKAGE_INSTALL",
    "CLAIRE_ALLOW_COMMAND_EXECUTION",
    "CLAIRE_ALLOW_BROWSER_EXECUTION",
    "CLAIRE_ALLOW_AUTOMATIC_UPDATES",
]
...
def _audit_paths(root: Path, report: dict[str, Any]) -> None:
    path_report: dict[str, Any] = {
        ...
        "frontend_assets": {},
        "active_tree_policy": {
            "uses_top_level_claire": (root / "claire").exists(),
    ...
    if (root / "src").exists():
        _record_issue(
            ...
            "A src/ tree exists even though current active tree should be top-level claire/, frontend/, tests/. Review for shadow imports before future builds.",
        )
...
def _audit_python_syntax(root: Path, report: dict[str, Any]) -> None:
    compile_roots = [root / "claire", root / "tests"]
...
def _build_test_client(report: dict[str, Any]):
    try:
        ...
        _record_issue(
            ...
            "blocker",
            "create_app_failed",
            "Could not import claire.app.create_app() or create TestClient.",
...
def _audit_action_labels(report: dict[str, Any]) -> None:
    ...
    stage_like = [label for label in labels if re.match(r"^S\d+(?:\s|[-–]|$)", label)]
...
def _audit_frontend_assets(root: Path, report: dict[str, Any]) -> None:
    ...
    required_mounts = [
        ...
        "operator_action_click_bridge.css",
    ]
    ...
    for rel, info in frontend.items():
        if info["contains_post_method"] or info["contains_dot_post"]:
...
def _audit_static_source_risks(root: Path, report: dict[str, Any]) -> None:
    ...
    active_roots = [root / "claire", root / "frontend"]
...
def _write_markdown(report: dict[str, Any], json_path: Path) -> Path:
    ...
    for path, data in routes.items():
        lines.append(f"- `{path}` → `{data.get('status_code', data.get('error'))}`")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return md_path
