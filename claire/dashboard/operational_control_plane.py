...
from claire.lifecycle.lifecycle_registry import CoreLifecycleRegistry
...
REQUIRED_OPERATIONAL_FILES = [
    ...
    "claire/dashboard/operational_control_plane.py",
    "claire/lifecycle/lifecycle_registry.py",
    "claire/lifecycle/threshold_provenance.py",
    "claire/data_lineage/source_record_builder.py",
    "claire/data_lineage/provenance_tracker.py",
...
def internet_authority_status() -> dict[str, Any]:
    selected_provider = os.environ.get("CLAIRE_SEARCH_PROVIDER", "").strip().lower() or "none"
    key_state = {
        key: {
            "present": bool(os.environ.get(key)),
            "redacted": "***" if os.environ.get(key) else "",
        }
        for key in INTERNET_ENV_KEYS
    }
    provider_key_present = any(key_state[key]["present"] for key in INTERNET_ENV_KEYS if key != "CLAIRE_SEARCH_PROVIDER")
...
def source_lineage_status() -> dict[str, Any]:
    lineage_files = [
        "claire/data_lineage/source_record_builder.py",
        "claire/data_lineage/provenance_tracker.py",
        "claire/data_lineage/citation_anchor_registry.py",
        "claire/data_lineage/transformation_chain.py",
        "claire/operational_evidence/lineage_record.py",
    ...
    return {
        ...
        "logger_available": _exists("claire/operational_evidence/lineage_record.py"),
        "provenance_tracker_available": _exists("claire/data_lineage/provenance_tracker.py"),
...
def operational_actions() -> list[dict[str, Any]]:
    return [
        ...
        {"id": "open_search_plans", "category": "Internet", "label": "Open Governed Search Plans", "method": "GET", "endpoint": "/api/search/governed/plans"},
