...
from claire.api.operator_cockpit_runtime import runtime_status_payload
from claire.api.industry_standard_endpoint_package import build_endpoint_standard_settings
from claire.api.endpoint_reconciliation_report import build_endpoint_reconciliation_report
from claire.api.dependency_chain_proof import PROOF_PATH as DEPENDENCY_CHAIN_PROOF_PATH
from claire.api.dashboard_active_control_map import build_dashboard_active_control_map
from claire.design.artifact_package import build_design_artifact_package
from claire.dashboard.system_wiring_map import build_system_wiring_map
from claire.design.live_design_portal import build_live_design_portal_workbench
from claire.validation.run_quality_gate import evaluate_run_quality
...
def configured_provider_state(root: Path) -> dict[str, Any]:
    ...
    provider_root = None
    try:
        from claire.api.governed_connected_search import project_root as connected_project_root
        from claire.api.governed_connected_search import provider_state
    ...
    if not configured_provider and provider_root is not None and root == provider_root:
        configured_provider = os.environ.get("CLAIRE_SEARCH_PROVIDER", "").strip().lower()
...
def acquirer_match_items(query: str = "", limit: int = 12) -> list[dict[str, Any]]:
    try:
        ...
        from claire.technology.technology_base import assess_technology_base
    except Exception:
        return []
...
def build_cockpit_dashboard_state(project_root: Path | str | None = None) -> dict[str, Any]:
    ...
    from claire.platform.update_governance.open_web_update_governance import build_dashboard_update_panel

    update_governance_panel = build_dashboard_update_panel(root)
    from claire.platform.intelligence_modes import build_intelligence_mode_state

    intelligence_modes = build_intelligence_mode_state(root)
    from claire.dashboard.cockpit_command_plan import command_history
    ...
    from claire.api.governed_connected_search import search_lane_config
    ...
    frontend_files_ready = all(
        ...
        for name in ("claire_dashboard.html", "claire_dashboard.css", "claire_dashboard.js")
    )
    backend_dashboard_files_ready = all(
        (root / path).exists()
        for path in (
            "claire/dashboard/cockpit_dashboard_state.py",
            "claire/dashboard/cockpit_command_plan.py",
            "claire/api/routes_pipeline.py",
            "claire/app.py",
        )
    )
    operator_button_files_ready = (root / "claire/api/operator_cockpit_runtime.py").exists()
    search_bar_files_ready = all(
        (root / path).exists()
        for path in (
            "claire/api/governed_connected_search.py",
            "claire/dashboard/cockpit_command_plan.py",
        )
    ...
    completion_items = [
        ...
        completion_item("dashboard backend state files", backend_dashboard_files_ready, 8, "claire/dashboard + claire/app.py"),
        completion_item("topbar action endpoints", operator_button_files_ready, 8, "claire/api/operator_cockpit_runtime.py"),
        ...
        completion_item("technology database binding", technology_base["counts"]["records"] > 0, 8, "claire/technology/technology_base.py"),
