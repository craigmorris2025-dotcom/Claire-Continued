...
CORE_OUTPUTS = {
    "active_dashboard": "frontend/command_center/modern/index.html",
...
def assess_dashboard(root: Path, reports: Dict[str, Any], core: Dict[str, Any]) -> Dict[str, Any]:
    ...
    js_file = root / "frontend/command_center/modern/claire_functional_operator_dashboard.js"
    ...
    required_words = ["Mission", "Routes", "Discovery", "AutoDesign", "Portfolio", "Acquisition", "Internet", "Updates", "Diagnostics"]
    missing_words = [word for word in required_words if word not in text]
    api_wired = "/operator/dashboard/state" in js or "/dashboard/state" in js
    return {
        "status": "passed" if core["active_dashboard"]["exists"] and api_wired and not missing_words else "blocked",
