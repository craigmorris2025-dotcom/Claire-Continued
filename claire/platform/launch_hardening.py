...
def import_check() -> Dict[str, Any]:
    ...
    try:
        from claire.app import app  # noqa: F401
        results["claire.app"] = {"status": "passed"}
    except Exception as exc:
        results["claire.app"] = {"status": "failed", "error": str(exc), "traceback": traceback.format_exc(limit=8)}
    return results
...
def launcher_check(root: Path) -> Dict[str, Any]:
    ...
    if not launcher.exists():
        return {"status": "failed", "errors": ["LAUNCH_CLAIRE.bat missing"], "warnings": []}
...
%PYTHON_EXE% -c "import claire.app; print('Claire app import: OK')" || (
    echo.
    echo Claire app import failed. Run the v17.77 tests and inspect the error.
    pause
    exit /b 1
)
...
start "Claire Backend v17.77" cmd /k "%PYTHON_EXE% -m uvicorn claire.app:app --host 127.0.0.1 --port 8000 --reload"
...
endlocal
"""
    path = root / "LAUNCH_CLAIRE.bat"
    path.write_text(launcher_text, encoding="utf-8")
    return {"status": "written", "path": "LAUNCH_CLAIRE.bat"}
...
def determine_stop_go(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if report["imports"].get("claire.app", {}).get("status") != "passed":
        blockers.append("claire_app_import_failed")
