...
CORE_STARTUP_FILES = {
    "primary_launcher": "LAUNCH_CLAIRE.bat",
...
def import_check() -> Dict[str, Any]:
    ...
    try:
        ...
        results["claire.app"] = {"status": "passed", "route_count": route_count}
    except Exception as exc:
...
def launcher_content_check(root: Path) -> Dict[str, Any]:
    ...
    for name in ["primary_launcher", "safe_launcher", "startup_verifier"]:
        ...
        lower = text.lower()
        checks[name] = {
            "status": "passed" if "claire.app:app" in lower or name == "startup_verifier" else "warning",
            "path": relative,
            "starts_uvicorn_claire_app": "uvicorn claire.app:app" in lower,
...
def determine_stop_go(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if primary.get("starts_uvicorn_claire_app") is not True:
        blockers.append("primary_launcher_does_not_start_uvicorn_claire_app")
    if safe.get("starts_uvicorn_claire_app") is not True:
