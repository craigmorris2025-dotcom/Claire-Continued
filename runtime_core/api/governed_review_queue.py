...
def _write_store(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        # Windows/OneDrive sandboxed test paths can intermittently reject a
        # large rewrite. Preserve runtime progress with a compact retry before
        # falling back to the caller's in-memory decision payload.
        compact_body = json.dumps(payload, separators=(",", ":"), sort_keys=True)
        try:
            tmp_path.write_text(compact_body, encoding="utf-8")
            tmp_path.replace(path)
        except OSError:
            if tmp_path.exists():
                try:
