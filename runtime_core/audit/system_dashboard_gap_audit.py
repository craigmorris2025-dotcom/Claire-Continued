...
def _analyze_capabilities(routes: list[dict[str, Any]], frontend: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    active_control_frontend_bound = all(
        term in all_text
        for term in [
            "/api/dashboard/active-control-map",
            "renderactivecontrols",
            "runactivecontrol",
            "claire-active-control-result-pane",
        ]
    )
    active_controls_by_key: dict[str, dict[str, Any]] = {}
    try:
        from runtime_core.api.dashboard_active_control_map import ACTIVE_CONTROL_MAP

        active_controls_by_key = {str(control.get("key")): control for control in ACTIVE_CONTROL_MAP}
    except Exception:
        active_controls_by_key = {}
    ...
    for capability in CAPABILITY_CATALOG:
        ...
        active_control = active_controls_by_key.get(capability["key"], {})
        active_control_endpoints = [
            endpoint
            for endpoint in [
                active_control.get("primary_endpoint"),
                *active_control.get("secondary_endpoints", []),
            ]
            if isinstance(endpoint, str) and endpoint
        ]
        active_control_routes = [
            route
            for route in routes
            if route["path"] in active_control_endpoints
        ]
        for route in active_control_routes:
        ...
        if active_control_frontend_bound and active_control:
            matching_fetches.append(
                {
                    "file": "frontend/command_center/modern/platform_dashboard.js",
                    "url": "/api/dashboard/active-control-map",
                    "resolves_control": str(active_control.get("primary_endpoint") or ""),
                }
            )
        matching_buttons = [
            button
        ...
        if active_control_frontend_bound and active_control:
            matching_buttons.append(
                {
                    "file": "frontend/command_center/modern/platform_dashboard.js",
                    "label": str(active_control.get("button_label") or active_control.get("label") or capability["label"]),
                    "attrs": f"data-active-control-key={capability['key']}",
                }
            )
...
def _markdown_report(report: dict[str, Any]) -> str:
    ...
    if not lagging:
        ...
        for cap in lagging:
            lines.append(f"- **{cap['label']}** â€” `{cap['status']}` â€” {cap['gap']}")
    lines.extend(["", "## Next recommended build", ""])
    next_build = report["next_recommended_build"]
    lines.append(f"- **{next_build['id']} â€” {next_build['name']}**")
