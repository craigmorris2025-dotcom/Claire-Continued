...
REQUIRED_LAUNCHER_TEXT = [
    "PLATFORM_ALLOW_CONTROLLED_HEAD_PROBE=1",
    "PLATFORM_ALLOW_CONTROLLED_METADATA_GET=1",
    "PLATFORM_ALLOW_CONTROLLED_LIMITED_BODY_GET=1",
    "PLATFORM_ALLOW_REAL_SEARCH_PROVIDER=1",
...
def inspect_single_dashboard_launcher_state(root: Path = ROOT) -> Dict[str, Any]:
    ...
    ready = (
        ...
        and root_bats == ["LAUNCH_PLATFORM.bat"]
...
def build_root_consolidation_report(root: Path = ROOT) -> Dict[str, Any]:
    ...
    return {
        ...
        "state": state,
        "operator_next_step": (
            "Run LAUNCH_PLATFORM.bat, wait for backend startup, use the dashboard search bar with google."
            if state["ready"] else
