...
def policy_from_environment(env: Mapping[str, str] | None = None) -> ConcurrentProbePolicy:
    ...
    Required manual flag:
        PLATFORM_ALLOW_SAFE_CONCURRENT_PROBES=1

    Optional limits:
        PLATFORM_MAX_CONCURRENT_PROBES defaults to 2 when enabled
        PLATFORM_MAX_CONCURRENT_PROBE_BATCH defaults to 5 when enabled
    """
    import os

    source: Mapping[str, str] = env if env is not None else os.environ
    enabled = _env_enabled(source.get("PLATFORM_ALLOW_SAFE_CONCURRENT_PROBES"))
    ...
    return ConcurrentProbePolicy(
        enabled=True,
        max_concurrent_probes=read_int("PLATFORM_MAX_CONCURRENT_PROBES", 2, 1, 4),
        max_batch_size=read_int("PLATFORM_MAX_CONCURRENT_PROBE_BATCH", 5, 1, 10),
...
def evaluate_safe_concurrency(
    ...
    return ConcurrentProbeDecision(
        ...
        governance_state={
            ...
            "manual_enable_flag": "PLATFORM_ALLOW_SAFE_CONCURRENT_PROBES",
