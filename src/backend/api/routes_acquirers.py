...
from backend.api.schemas import AcquirersResponse, AcquirerProfile
...
async def list_acquirers(sector: str = None):
    ...
    acquirers = []
    for p in profiles:
        acquirers.append(AcquirerProfile(
            name=p.get("name", ""),
            ticker=p.get("ticker", ""),
            sector=p.get("sector", ""),
            fit=p.get("fit", 0),
    ...
    return AcquirersResponse(acquirers=acquirers, count=len(acquirers))
...
async def get_acquirer(ticker: str):
    ...
    match = [p for p in profiles if p.get("ticker", "").upper() == ticker.upper()]
    if not match:
        raise HTTPException(status_code=404, detail=f"Acquirer {ticker} not found")
    return match[0]
