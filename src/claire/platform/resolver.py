...
from claire.platform.manifest import gap_analysis, TARGET_STATE
...
from claire.platform.manifest import current_state, gap_analysis, TARGET_STATE
from claire.platform.resolver import resolve_all, resolve_one
...
def _update_server_routes(root: str = ".") -> Dict[str, Any]:
    ...
    if "routes_platform" not in content:
        # Add import
        content = content.replace(
            "from claire.api.routes_proxy import router as proxy_router",
            "from claire.api.routes_proxy import router as proxy_router\n"
            "    from claire.api.routes_platform import router as platform_router"
        )
        # Add registration
        content = content.replace(
