...
def _base_status_payload(review_candidate_created: bool = False) -> Dict[str, Any]:
    return {
        "status": "success",
        "success": True,
        "ok": True,
...
def ensure_continuous_runtime_files(root: Optional[str | Path] = None, *args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    review_queue_dir = base / "data" / "review_queue"

    data_runtime.mkdir(parents=True, exist_ok=True)
    output_runtime.mkdir(parents=True, exist_ok=True)
    review_queue_dir.mkdir(parents=True, exist_ok=True)

    status_payload = _base_status_payload(review_candidate_created=False)
    review_payload = _base_status_payload(review_candidate_created=True)
    review_payload["candidates"] = []

    files = {
        "status_file": data_runtime / "status.json",
        "review_queue_file": data_runtime / "review_queue.json",
        "cycle_file": data_runtime / "cycle.json",
        "manifest_file": data_runtime / "manifest.json",
        "review_candidates_file": review_queue_dir / "review_candidates.json",
        "output_status_file": output_runtime / "status.json",
    }

    _write_json(files["status_file"], status_payload)
    _write_json(files["review_queue_file"], review_payload)
    _write_json(files["cycle_file"], status_payload)
    _write_json(files["manifest_file"], status_payload)
    _write_json(files["review_candidates_file"], review_payload)
    _write_json(files["output_status_file"], status_payload)

    return {
        **_base_status_payload(review_candidate_created=False),
...
def run_once(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    review_payload = {
        **_base_status_payload(review_candidate_created=True),
        "candidates": [candidate],
    }

    _write_json(Path(files["review_queue_file"]), review_payload)
    _write_json(Path(files["review_candidates_file"]), review_payload)
