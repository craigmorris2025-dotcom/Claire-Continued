...
class ContinuousRuntimeCycleResult:
    ...
    review_candidate_created: bool = False
    ...
    def to_payload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "review_candidate_created": self.review_candidate_created,
...
class ContinuousRuntimeCycle:
    ...
    def run(self, request: Optional[Mapping[str, Any]] = None, **kwargs: Any) -> ContinuousRuntimeCycleResult:
        ...
        result_payload.setdefault("completed", "success")
        result_payload.setdefault("review_candidate_created", True)
        result_payload.setdefault("loop_running", False)
        result_payload.setdefault("runtime_truth_mutated", False)
        result_payload.setdefault("promotion_required", True)

        return ContinuousRuntimeCycleResult(
            status=str(result_payload.get("status") or "completed"),
            completed=str(result_payload.get("completed") or "success"),
            terminal_state=str(result_payload.get("terminal_state") or "insufficient_data"),
            ...
            review_candidate_created=bool(result_payload.get("review_candidate_created", True)),
            loop_running=bool(result_payload.get("loop_running", False)),
...
def ensure_continuous_runtime_files(root: Optional[str | Path] = None, *args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    root_runtime_dir = base / "runtime" / "continuous_runtime"
    root_review_dir = base / "review_queue"
    ...
    files = {
        ...
        "root_runtime_state_file": root_runtime_dir / "continuous_runtime_state.json",
        "root_runtime_cycle_file": root_runtime_dir / "continuous_runtime_cycle.json",
        "root_review_candidates_file": root_review_dir / "review_candidates.json",
        ...
        "legacy_manifest_file": output_dir / "continuous_runtime_manifest.json",
    }

    for name, path in files.items():
        payload = candidate_payload if "review_candidates" in name else state_payload
    ...
    return {
        ...
        "root_runtime_dir": str(root_runtime_dir),
        "root_review_dir": str(root_review_dir),
...
def _coerce_legacy_args_to_request(*args: Any, **kwargs: Any) -> tuple[Dict[str, Any], Optional[Path]]:
    ...
    for arg in args:
        if isinstance(arg, Mapping):
            ...
            text = str(arg)
            if isinstance(arg, Path) or any(sep in text for sep in ["/", "\\"]) or text in {".", ".."}:
                root = Path(text)
    ...
    for root_key in ["root", "base_dir", "project_root"]:
