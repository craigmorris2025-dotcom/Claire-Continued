...
def ensure_continuous_runtime_files(root: Optional[str | Path] = None, *args: Any, **kwargs: Any) -> Dict[str, Any]:
    """Exact v19.84.2 compatibility contract.

    Required by tests/test_v19_84_2_continuous_runtime_cycle.py:
    - creates root/data/continuous_runtime/status.json
...
def _coerce_args(*args: Any, **kwargs: Any) -> Tuple[Path, Dict[str, Any]]:
    ...
    if args:
        ...
        if isinstance(first, (str, Path)):
            ...
            # v19.84.2 calls run_once(root, "text")
            if isinstance(first, Path) or first_path.exists() or str(first) in {".", ".."} or "\" in str(first) or "/" in str(first):
                root = first_path
                args = args[1:]
...
def run_once(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    - result["continuous_runtime_status"] == "configured_not_running"
    - result must not claim loop_running
    """
