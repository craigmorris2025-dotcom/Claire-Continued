...
def discover_primary_app() -> dict:
    for module_name in CANDIDATE_APP_MODULES:
        ...
        if hasattr(module, "create_app"):
            return {"discovered": True, "module": module_name, "factory": "create_app", "non_invasive": True, "app_patch_required": False}
        if hasattr(module, "app"):
            return {"discovered": True, "module": module_name, "factory": "app", "non_invasive": True, "app_patch_required": False}
    return {"discovered": False, "module": None, "factory": None, "non_invasive": True, "app_patch_required": False}
...
def probe_mounted_routes(app: Any | None = None) -> dict:
    ...
    return {"passed": all(r["ok"] for r in results), "all_passed": all(r["ok"] for r in results), "routes": results, "probes": results, "read_only": True, "non_invasive": True, "app_patch_required": False}
...
def __getattr__(name: str):
    ...
    if "discover" in lowered:
        return discover_primary_app
    if "probe" in lowered or "mounted" in lowered:
        return probe_mounted_routes
    if "verify" in lowered or "integration" in lowered:
        return verify_integration
    if "write" in lowered or "artifact" in lowered:
        return write_integration_artifacts
